...
Riêng với GRU4Rec, chúng tôi tách thêm một tập \textbf{validation} bằng cách cắt phần \emph{muộn nhất} của training set (theo cùng nguyên tắc thời gian) để phục vụ early stopping. Như vậy thứ tự thời gian luôn là train-inner $<$ validation $<$ test. Popularity và SKNN không cần validation nên vẫn dùng toàn bộ training set; điều này giữ cho kết quả của chúng và tập test hoàn toàn không đổi.
...
\quad train-inner (GRU4Rec) & -- & 96,234 \\
\quad validation (GRU4Rec) & -- & 12,029 \\
Số phiên test & -- & 8,004 \\
\bottomrule
\end{tabular}
\end{table}
...
& Top-N & 20 \\
\midrule
\multirow{11}{*}{GRU4Rec} & Embedding size ($d$) & 64 \\
& Hidden size ($H$) & 128 \\
& Số lớp GRU & 1 \\
& Dropout & 0.25 \\
& Optimizer & Adam \\
...
\begin{itemize}
    ...
    \item \textbf{Embedding size 64 và hidden size 128:} Đây là cấu hình vừa phải cho GRU4Rec: đủ năng lực học biểu diễn item và trạng thái phiên, nhưng không làm số tham số quá lớn so với mẫu 1/64. Với vocab $M=10{,}234$ (xây trên tập train-inner), mô hình có khoảng 2.05 triệu tham số, phù hợp mục tiêu thực nghiệm minh họa.
    ...
    \item \textbf{Số epoch và early stopping:} Thay vì cố định số epoch, chúng tôi tách một tập validation từ phần cuối (muộn nhất) của tập train và dùng \emph{early stopping}: mỗi epoch đánh giá Recall@20 trên validation, lưu lại trọng số tốt nhất và dừng huấn luyện nếu chỉ số này không cải thiện sau 2 epoch (patience = 2). Số epoch tối đa đặt là 40. Trong thực nghiệm, Recall@20 trên validation đạt đỉnh tại epoch 19 và quá trình dừng ở epoch 21; mô hình cuối dùng trọng số của epoch 19. Cách làm này chọn số epoch một cách hệ thống và cho ước lượng hiệu năng test không thiên vị (không ``nhìn'' vào test để chọn epoch).
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Lọc độ dài phiên $[2, 20]$:} Sau khi bỏ item hiếm khỏi mọi phiên, giữ lại các phiên có từ 2 đến 20 item. Loại phiên 1 item (không tạo được cặp input--label) và phiên quá dài $>20$ item (có thể là bot/outlier). Kết quả cuối: train-full 108,263 phiên, test 8,004 phiên.

    \item \textbf{Tách validation cho GRU4Rec:} Từ tập train-full (108,263 phiên), cắt $1/9$ phần \emph{muộn nhất} theo thời gian làm validation (12,029 phiên), phần còn lại làm train-inner (96,234 phiên). Popularity và SKNN vẫn dùng train-full nên kết quả của chúng không đổi; chỉ GRU4Rec huấn luyện trên train-inner và dùng validation cho early stopping. Vì vocab của GRU4Rec được xây trên train-inner nên kích thước từ vựng là 10,234 (gồm token padding).
\end{enumerate}
...
\begin{itemize}
    ...
    \item MRR@20 = 0.1350: Trung bình, item đúng nằm ở khoảng vị trí thứ 7 ($1/0.1349 \approx 7.4$) trong danh sách Top-20.
...
\subsection{Kết quả GRU4Rec (early stopping, best epoch 19/40)}

\begin{table}[H]
\centering
\caption{Kết quả GRU4Rec với early stopping (best epoch 19/40)}
\label{tab:gru4rec_results}
\begin{tabular}{lcc}
\toprule
\textbf{Mô hình} & \textbf{Recall@20} & \textbf{MRR@20} \\
\midrule
GRU4Rec (best epoch 19/40) & \textbf{0.2287} (22.87\%) & \textbf{0.1043} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Nhận xét:}
\begin{itemize}
    \item Recall@20 = 22.87\%: Cải thiện \textbf{+21.00 điểm} so với Popularity, nhưng \textbf{thấp hơn SKNN 6.65 điểm}.
    \item MRR@20 = 0.1043: Thấp hơn SKNN 0.0307 (SKNN xếp hạng item đúng cao hơn).
    \item Số epoch được chọn bằng \textbf{validation + early stopping}: Recall@20 trên validation đạt đỉnh 0.2739 tại epoch 19, quá trình dừng tại epoch 21 (patience = 2). Đây là ước lượng hiệu năng test \emph{không thiên vị}, khác với việc huấn luyện cứng một số epoch rồi nhìn thẳng vào test để chọn.
    \item Trên cấu hình thí nghiệm này (mẫu 1/64, chia theo thời gian, train-inner ít hơn train-full do tách validation), GRU4Rec \textbf{chưa vượt} SKNN. Kết quả này phù hợp với Ludewig \& Jannach (2018) \cite{ludewig2018}: SKNN là baseline cực mạnh mà nhiều mô hình deep learning khó vượt qua, nhất là khi dữ liệu hạn chế.
    \item GRU4Rec vẫn có lợi thế lý thuyết (\textbf{học được thứ tự} click) và có thể cải thiện thêm khi tăng dữ liệu hoặc tinh chỉnh siêu tham số -- phân tích chi tiết ở Mục \ref{chap:analysis}.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{K=50:} Chỉ xét 50 phiên tương tự nhất. Một số item phù hợp có thể chưa nhận đủ đóng góp từ các phiên hàng xóm, dẫn đến Recall thấp nhất trong dải thử nghiệm (0.2704).
...
\subsection{Đường cong huấn luyện và early stopping của GRU4Rec}

Hình \ref{fig:loss_curve} thể hiện hai đường cong theo epoch: loss huấn luyện (trên) và Recall@20 trên tập validation (dưới). Vạch đứng đánh dấu \emph{best epoch} (epoch 19) được early stopping chọn.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{gru4rec_loss_curve.png}
\caption{Loss huấn luyện và Recall@20 trên validation của GRU4Rec theo epoch; vạch đỏ là best epoch (19) do early stopping chọn. Nguồn: kết quả thực nghiệm của nhóm từ \texttt{output/all\_results.pkl}, trực quan hóa bằng \texttt{src/plots.py}.}
...
\begin{enumerate}
    \item \textbf{Loss huấn luyện giảm đơn điệu:} từ 7.84 (epoch 1) xuống 3.90 (epoch 21), không bao giờ tăng trở lại. Nếu chỉ nhìn loss train, ta sẽ tưởng rằng cứ huấn luyện thêm là tốt hơn.

    \item \textbf{Recall@20 trên validation bão hòa:} tăng nhanh ở các epoch đầu (0.106 $\to$ 0.260 trong 10 epoch đầu), sau đó chững lại và \emph{bão hòa} quanh 0.272--0.274 từ epoch 15 trở đi, đạt đỉnh 0.2739 tại epoch 19.

    \item \textbf{Early stopping kích hoạt:} sau epoch 19, hai epoch tiếp theo (20--21) không vượt được đỉnh nên quá trình dừng tại epoch 21 (patience = 2) và khôi phục trọng số của epoch 19.

    \item \textbf{Bài học:} loss huấn luyện giảm \emph{không} đồng nghĩa chất lượng trên dữ liệu chưa thấy tiếp tục tăng. Đây chính là lý do cần một tập validation độc lập để chọn epoch, thay vì dựa vào loss huấn luyện hay cố định số epoch một cách tùy ý.
\end{enumerate}

\textbf{Gợi ý cải thiện thêm:}
\begin{itemize}
    \item Tăng lượng dữ liệu huấn luyện (giảm tỉ lệ lấy mẫu) để mô hình học biểu diễn tốt hơn
    \item Sử dụng learning rate scheduling (giảm lr khi val bão hòa)
    \item Tinh chỉnh hidden size, dropout, weight decay qua tìm kiếm siêu tham số trên validation
\end{itemize}
...
SKNN (K=500) & \textbf{0.2952} & \textbf{0.1350} & +0.2765 & $\sim$2s & Không \\
GRU4Rec (best ep 19/40) & 0.2287 & 0.1043 & +0.2100 & $\sim$25 phút & Có \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item SKNN cao hơn GRU4Rec (+6.65 điểm Recall, +0.0307 MRR) trên cấu hình thí nghiệm này.
    \item Việc SKNN vượt GRU4Rec là kết quả thường gặp, phù hợp với các nghiên cứu trước \cite{ludewig2018, jannach2017}, nhất là khi dữ liệu hạn chế và mô hình được đánh giá nghiêm ngặt bằng validation thay vì nhìn thẳng vào test.
\end{enumerate}
...
Trên cấu hình thí nghiệm này, GRU4Rec thấp hơn SKNN 6.65 điểm Recall. Tuy nhiên, về \emph{lý thuyết} GRU4Rec vẫn có những ưu thế quan trọng mà điều kiện thí nghiệm hạn chế (mẫu 1/64, tập train-inner nhỏ hơn do tách validation) chưa cho phép phát huy:
...
\begin{enumerate}
    ...
    \item \textbf{Dữ liệu hạn chế:} Chỉ dùng mẫu 1/64, và GRU còn huấn luyện trên train-inner ($\sim$96K phiên, ít hơn train-full sau khi tách validation) -- chưa đủ để học tốt embedding cho hơn 10K item. Trên toàn bộ dataset, GRU thường thu hẹp hoặc vượt khoảng cách này.

    \item \textbf{Đã hội tụ trên validation:} Khác với việc chỉ giảm loss, Recall@20 trên validation của GRU4Rec đã \emph{bão hòa} quanh epoch 15--19 (đỉnh 0.2739) rồi không cải thiện thêm, nên early stopping dừng tại epoch 21. Tức là với cấu hình hiện tại, tăng thêm epoch \emph{không} còn giúp ích; muốn cải thiện cần thêm dữ liệu hoặc tinh chỉnh kiến trúc/siêu tham số, không phải chỉ kéo dài huấn luyện.
...
\begin{enumerate}
    ...
    \item \textbf{GRU4Rec mới được huấn luyện ở mức vừa phải:} Số epoch được chọn bằng validation + early stopping (best epoch 19), nhưng chưa tune rộng learning rate, dropout, batch size hay loss; ngoài ra mô hình huấn luyện trên train-inner nhỏ hơn train-full do phải tách validation. Vì vậy việc GRU4Rec thấp hơn SKNN trong thí nghiệm này không phủ định tiềm năng của mô hình tuần tự sâu; nó chỉ phản ánh cấu hình đang xét.
...
\begin{enumerate}
    ...
    \item \textbf{SKNN với K=500} (Recall@20 = 29.52\%, MRR@20 = 0.1350): Phương pháp dựa trên phiên lân cận đạt kết quả cao nhất trong thí nghiệm, cao hơn Popularity +27.65 điểm Recall@20. Điểm mạnh của SKNN là không cần huấn luyện và có thể diễn giải qua các phiên hàng xóm.

    \item \textbf{GRU4Rec (best epoch 19/40, early stopping)} (Recall@20 = 22.87\%, MRR@20 = 0.1043): Mô hình học sâu \textbf{chưa vượt} SKNN (thấp hơn 6.65 điểm Recall) trong cấu hình mẫu 1/64. Số epoch được chọn một cách hệ thống bằng validation + early stopping (đỉnh Recall@20 trên val tại epoch 19), cho ước lượng hiệu năng test không thiên vị. Kết quả này hợp lý vì GRU4Rec thường cần nhiều dữ liệu và tune siêu tham số kỹ hơn.
\end{enumerate}
...
\begin{itemize}
    ...
    \item Việc dùng validation + early stopping (thay vì cố định số epoch) là một thực hành đánh giá nghiêm túc: đường cong cho thấy loss huấn luyện vẫn giảm trong khi Recall@20 trên validation đã bão hòa, nên không thể chọn epoch chỉ dựa vào loss huấn luyện.
\end{itemize}
